  ...
  <div class="cart">
    <order-header title="我的购物车">
      <template v-slot:tips>
        <span>温馨提示：产品是否购买成功，以最终下单为准，请尽快结算噢</span>
      </template>
    </order-header>
    <div class="wrapper">
      <div class="container">
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item,index) in list" :key="index">
              ...
              <div class="item-del" @click="confirmDelProduct(item)"></div>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    ...
    // 删除购物车商品
    confirmDelProduct(item) {
      this.showModal = true;
      this.delItemid = item.productId;
    },
    delProduct() {
      console.log(this.delItemid);
      this.axios.delete(`/carts/${this.delItemid}`).then(res => {
        this.renderData(res);
